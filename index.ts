...
let myamount = 20000; // my total amount save in variable
let myPin = 9876;   //  my pincode save in variable

console.log(chalk.red.bold.italic("welcome"));// welcome notice
...
let answers = await inquirer.prompt([      // ask user stop and check mark
{
    name : "language",                           
    message : "please select a language", // in this object choose a language
    type : "list",                            
...
if(answers.userpin === myPin ){
    console.log("your pin is correct")} // in console agar pin barabr ha tu ok otherwise stop


let accountType = await inquirer.prompt([ // again user wait nd check mark
    {
           name:"accountType",
...
let operationAns = await inquirer.prompt([    // again user wait nd check mark
...
{
    let withdraw = await inquirer.prompt([{   // user stop nd add amount
       name : "amount",
...
if(withdraw.amount > myamount){           // agar amount zayada ha mary amount sa
    console.log(chalk.red.bold.italic("insufficent amount"))}; // ask no balance
